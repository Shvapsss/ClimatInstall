...
class plgSystemRSFPReCaptchav2 extends JPlugin
{
	...
	public function rsfp_bk_onAfterShowComponents() {
		...
		JFactory::getDocument()->addStyleDeclaration("ul.rsform_leftnav li a#recaptchav2 span {
				background: url('".JURI::root(true)."/administrator/components/com_rsform/assets/images/icons/recaptchav2.png') no-repeat scroll 10px center transparent;
		}");
		?>
		<li class="rsform_navtitle"><?php echo JText::_('RSFP_RECAPTCHAV2_LABEL'); ?></li>
		<li><a href="javascript: void(0);" onclick="<?php echo $link;?>;return false;" id="recaptchav2"><span><?php echo JText::_('RSFP_RECAPTCHAV2_LABEL'); ?></span></a></li>
		<?php
	}
	...
	public function rsfp_bk_onAfterCreateFrontComponentBody($args) {
		...
		if ($typeId == 2424) {
			...
			if (!$loaded) {
				...
				// This must be added as a custom tag because it needs to be placed after <scripts>
				$doc->addCustomTag('<script type="text/javascript" src="https://www.google.com/recaptcha/api.js?onload=RSFormProReCAPTCHAv2onLoad&amp;render=explicit'.$hl.'" defer async></script>');
