...
class plgSystemRSFPPayment extends JPlugin
{
	...
	public function rsfp_bk_onAfterShowComponents() {
		...
		<li><a href="javascript: void(0);" onclick="<?php echo $link4;?>;return false;" id="rsfpc28"><span id="donation"><?php echo JText::_('RSFP_DONATION'); ?></span></a></li>
			...
			case 'multipleProducts':
				...
				if ($data['VIEW_TYPE'] == 'DROPDOWN') {
					...
					foreach ($items as $item) {
						...
						$out .= '<input '.$additional.' type="'.$type.'" value="'.$this->_escape($txt).'" /><label>'.$txt_price.'</label>';
		...
		if ($params = $db->loadResult())
		{
			...
			if (is_object($params) && isset($params->UserEmail))
			{
				...
				if ($status = $db->loadObject())
				{
					...
					if ($app->isAdmin() && $task == 'submissions.resend') {
						// nothing for the resend button
					} else {
						// defer sending if
